...
namespace nextCloudAndroid
{
	public class DocumentFragment : Fragment
	{
		...
		protected TextView document_text;
		protected ImageView document_logo;
		...
		{
			View rootView = inflater.Inflate(Resource.Layout.fragment_document, container, false);
			...
			document_text = rootView.FindViewById<TextView> (Resource.Id.mesdocuments_text);
			loadfile = rootView.FindViewById<ProgressBar> (Resource.Id.loading_files);
			document_logo = rootView.FindViewById<ImageView> (Resource.Id.mesdocuments_logo);
		...
		public async void AfficherDocument(List<Fichier> list)
		{
			var query = ParseObject.GetQuery ("File").Where (filez => filez.Get<ParseUser> ("user") == ParseUser.CurrentUser && (filez.Get<String> ("type") == ".doc" || filez.Get<String> ("type") == ".docx" || filez.Get<String> ("type") == ".ppt" || filez.Get<String> ("type") == ".pptx" || filez.Get<String> ("type") == ".txt" || filez.Get<String> ("type") == ".xls" || filez.Get<String> ("type") == ".xlsx" || filez.Get<String> ("type") == ".pdf" || filez.Get<String> ("type") == ".zip" || filez.Get<String> ("type") == ".rar") && filez.Get<String> ("refer") == "");
			...
			if (nbr > 0)
			{
				...
				document_logo.Visibility = ViewStates.Gone;
				document_text.Visibility = ViewStates.Gone;
			}
			else if (nbr == 0)
			{
				loadfile.Visibility = ViewStates.Gone;
				document_logo.Visibility = ViewStates.Visible;
				document_text.Text = "No documents available !";
				document_text.Visibility = ViewStates.Visible;
			}
			...
			foreach (var filez in results)
			{
				...
				if (filez.Get<String> ("type") == ".doc" || filez.Get<String> ("type") == ".docx") {
