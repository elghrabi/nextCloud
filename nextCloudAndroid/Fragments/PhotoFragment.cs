...
namespace nextCloudAndroid
{
	public class PhotoFragment : Fragment
	{
		...
		protected ImageView photo_logo;
		...
		{
			View rootView = inflater.Inflate(Resource.Layout.fragment_photo, container, false);
			...
			photo_text = rootView.FindViewById<TextView> (Resource.Id.mesphotos_text);
			loadfile = rootView.FindViewById<ProgressBar> (Resource.Id.loading_files);
			photo_logo = rootView.FindViewById<ImageView> (Resource.Id.mesphotos_logo);
		...
		public async void AfficherImage(List<Fichier> list)
		{
			var query = ParseObject.GetQuery ("Fichier").Where (filez => filez.Get<ParseUser> ("user") == ParseUser.CurrentUser && (filez.Get<String> ("type") == ".jpg" || filez.Get<String> ("type") == ".png" || filez.Get<String> ("type") == ".jpeg" || filez.Get<String> ("type") == ".gif" || filez.Get<String> ("type") == ".bmp") && filez.Get<String> ("refer") == "");
			...
			if (nbr > 0)
			{
				...
				photo_logo.Visibility = ViewStates.Gone;
				photo_text.Visibility = ViewStates.Gone;
			}
			else if (nbr == 0)
			{
				loadfile.Visibility = ViewStates.Gone;
				photo_logo.Visibility = ViewStates.Visible;
				photo_text.Text = "No pictures available !";
				photo_text.Visibility = ViewStates.Visible;
			}
