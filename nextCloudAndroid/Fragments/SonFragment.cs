...
{
	public class SonFragment : Fragment
	{
		...
		protected TextView son_text;
		protected ImageView son_logo;
		...
		{
			View rootView = inflater.Inflate(Resource.Layout.fragment_son, container, false);
			...
			son_text = rootView.FindViewById<TextView> (Resource.Id.messons_text);
			loadfile = rootView.FindViewById<ProgressBar> (Resource.Id.loading_files);
			son_logo = rootView.FindViewById<ImageView> (Resource.Id.messons_logo);
			...
			folder_lay.Click += delegate
			{
				...
				alert.SetPositiveButton("Créer", new EventHandler<DialogClickEventArgs>(delegate {
					popup_new.Animate().TranslationY(300);
		...
		public async void AfficherSon(List<Fichier> list)
		{
			var query = ParseObject.GetQuery ("Fichier").Where (filez => filez.Get<ParseUser> ("user") == ParseUser.CurrentUser && (filez.Get<String> ("type") == ".mp3" || filez.Get<String> ("type") == ".wav" || filez.Get<String> ("type") == ".aac" || filez.Get<String> ("type") == ".m4a" || filez.Get<String> ("type") == ".ogg" || filez.Get<String> ("type") == ".wma" || filez.Get<String> ("type") == ".rm") && filez.Get<String> ("refer") == "");
			...
			if (nbr > 0)
			{
				...
				son_logo.Visibility = ViewStates.Gone;
				son_text.Visibility = ViewStates.Gone;
			}
			else if (nbr == 0)
			{
				loadfile.Visibility = ViewStates.Gone;
				son_logo.Visibility = ViewStates.Visible;
				son_text.Text = "No sounds available !";
				son_text.Visibility = ViewStates.Visible;
			}
			...
			foreach (var filez in results)
			{
				...
				if (filez.Get<String> ("type") == ".mp3" || filez.Get<String> ("type") == ".wav" || filez.Get<String> ("type") == ".aac" || filez.Get<String> ("type") == ".m4a" || filez.Get<String> ("type") == ".ogg" || filez.Get<String> ("type") == ".wma" || filez.Get<String> ("type") == ".rm") {
